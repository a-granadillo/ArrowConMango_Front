...
/// Validation tests for the 15 level definitions.
///
/// Ensures every level has a valid structure, respects the arrow-count
/// budgets per difficulty, has unique IDs, and can be mapped to domain
/// entities without collisions.
void main() {
  ...
  group('Arrow structure', () {
    test('every_arrow_should_have_1_to_3_nodes', () {
      for (final level in LevelDefinitions.allLevels) {
        for (final arrow in level.boardState.arrows) {
          expect(
            arrow.nodes.length,
            inInclusiveRange(1, 3),
            reason: 'Level ${level.id} arrow ${arrow.id} has invalid length',
          );
        }
      }
    });

    test('every_arrow_should_have_a_valid_direction', () {
      const validDirections = {'up', 'down', 'left', 'right'};
      for (final level in LevelDefinitions.allLevels) {
        for (final arrow in level.boardState.arrows) {
          expect(
            validDirections,
            contains(arrow.direction),
